"""
Database models for AutoCrypto Trader
"""
...
class FundSnapshot(Base):
    ...
    locked_25pct    = Column(Numeric(20, 8), default=0)      # accumulated locked profits
    in_trades       = Column(Numeric(20, 8), default=0)
    starting_fund   = Column(Numeric(20, 8), nullable=False) # baseline at snapshot day start
    pnl_today       = Column(Numeric(20, 8), default=0)
    pnl_total       = Column(Numeric(20, 8), default=0)
    milestone_hit   = Column(Boolean, default=False)         # 100% profit milestone flag
    snapshot_at     = Column(DateTime(timezone=True), server_default=func.now(), index=True)

    __table_args__ = (
        Index("ix_fund_snapshots_date", "snapshot_at"),
    )
...
class Signal(Base):
    ...
    id           = Column(Integer, primary_key=True)
    source       = Column(SAEnum(SignalSource), default=SignalSource.TRADINGVIEW)
    direction    = Column(SAEnum(TradeDirection), nullable=False)
    pair         = Column(String(20), nullable=False, index=True)
    price        = Column(Numeric(20, 8), nullable=False)
    atr          = Column(Numeric(20, 8), nullable=True)  # for stop-loss calc
    raw_payload  = Column(Text, nullable=True)            # original webhook JSON
...
class Trade(Base):
    ...
    # fund state at trade time
    fund_at_entry   = Column(Numeric(20, 8), nullable=True)
    fund_at_exit    = Column(Numeric(20, 8), nullable=True)
    ...
    __table_args__ = (
        Index("ix_trades_status_pair", "status", "pair"),
    )
...
class DailyReport(Base):
    ...
    report_json     = Column(Text, nullable=True)  # full JSON snapshot
    created_at      = Column(DateTime(timezone=True), server_default=func.now())
...
class Alert(Base):
    ...
    category    = Column(String(50), nullable=False)   # e.g. "auth", "fund", "api", "trade"
